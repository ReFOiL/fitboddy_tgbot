...
Revision ID: 41a232a48aed
Revises: 
Create Date: 2026-04-19 19:30:30.825387
...
def upgrade() -> None:
    ...
    sa.Column('training_load_multiplier', sa.Float(), server_default='1.0', nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'scheduled_workout_id', name='uq_workout_feedback_user_scheduled')
    )
    op.create_index(op.f('ix_workout_feedback_scheduled_workout_id'), 'workout_feedback', ['scheduled_workout_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_workout_feedback_scheduled_workout_id'), table_name='workout_feedback')
